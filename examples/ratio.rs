...
fn main() {
    ...
    for &level in &[1i32, 3, 6, 9, 13, 16, 17, 18, 19, 22] {
        ...
        println!(
            "{:<13} {:>10} {:>10} {:>10} {:>9}",
            "profile", "raw", "zstd_pure", "libzstd", "ratio"
        );
        for (name, data) in &profiles {
            ...
            let back = zstd::bulk::decompress(&compress(data, level, false, true), data.len() + 64)
                .unwrap();
            assert_eq!(
                &back, data,
                "{name}: our L{level} frame failed libzstd round-trip"
            );
            println!(
                "{:<13} {:>10} {:>10} {:>10} {:>8.2}x",
                name,
