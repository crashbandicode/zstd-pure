...
fn validate_frame(buf: &[u8], magicless: bool, label: &str) -> (usize, FrameHeader) {
    ...
    loop {
        ...
        assert!(
            pos + body <= buf.len(),
            "[{label}] block body runs past EOF"
        );
        pos += body;
        blocks += 1;
        if last {
...
fn check_single(
    ...
    assert_eq!(
        consumed,
        frame.len(),
        "[{label}] trailing bytes after a single frame"
    );
    ...
    assert_eq!(
        h.content_size, content_size,
        "[{label}] content-size pledge mismatch"
    );
    h
}
...
fn validate_stream(buf: &[u8], label: &str) -> usize {
    ...
    while pos < buf.len() {
        assert!(
            pos + 4 <= buf.len(),
            "[{label}] dangling bytes at stream end"
        );
        ...
        for &level in &[1i32, 9, 19] {
            for &checksum in &[false, true] {
                for &magic in &[true, false] {
                    ...
                    check_single(&frame, !magic, checksum, 0, Some(data.len() as u64), &label);
...
fn compress_long_is_well_formed() {
    ...
    for &checksum in &[false, true] {
        ...
        assert!(
            h.window_size >= data.len() as u64,
            "[{label}] window covers content"
        );
...
fn compress_seekable_emits_valid_frames() {
    ...
    assert_eq!(
        ...
        data,
        "seekable whole-stream round-trip"
    );
}
