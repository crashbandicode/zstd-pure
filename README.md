...
- [x] Streaming encoder (`StreamingEncoder`, `encode::stream`) — incremental, block-by-block frame encoding mirroring `StreamingDecoder` / zstd's `ZSTD_compressStream`: `push` bytes in arbitrary chunks, a compressed block is emitted per 128 KiB, the **unknown-content-size** frame header is written up front, and the XXH64 checksum on `finish`. Reuses the `compress` block loop (`Finder` + `EncState` + per-block store fallback) unchanged, parsing each block against only the input committed up to its boundary — so the produced frame is **byte-identical regardless of the write-chunk sizes** and decodes through our one-shot decoder, our `StreamingDecoder`, and libzstd. It implements the crate's `io::Write` (a `std::io::Write` re-export under `std`, a small shim under `no_std`), symmetric to `StreamingDecoder`'s `io::Read`, so it drops into `io::copy` / `write!`. **Memory is bounded** (~2× window + a couple of blocks, independent of total input): once the retained history passes `2 × window` the dead prefix is dropped and the finder is rebuilt over the retained window via the dictionary-priming path — uniform across every strategy (incl. the binary tree), reusing tested code so it can't reintroduce a position-aliasing bug. **Long-distance matching** is available via `with_options_long` (*experimental*): a coarse content-defined index contributes far matches and the frame advertises a larger window to admit them — the streaming analogue of `compress_long`; it stays bounded the same way (a slide rebuilds both the finder and the index over the retained window), at a larger window cost — **T4.1**
- [x] Parallel compression (`compress_parallel`, `encode::stream`'s sibling `encode::parallel`, `std`-only) — the independent-frame model (zstd `--threads`/ZSTDMT): split the input into `n_jobs` contiguous segments, compress each as a full frame on a `std::thread` scoped worker, and concatenate into a standard **multi-frame** stream our decoder + libzstd read end-to-end. **No new runtime deps**; gated on `std` (the `no_std` build never sees it). **Deterministic** (segments assigned by fixed offsets, reassembled in order). Loses cross-segment matching at the seams (a small ratio cost, like libzstd's job model); small inputs fall back to one frame — **T4.3**
